...
non.inf.edges <- setdiff(seq_len(as.edgelist(sim)), inf.edges)
sim2 <- sim
...
## improve Vsupp
ids.nvsupp.deg1 <- intersect(setdiff(all.inf, ids.vsupp),
                             which(EpiModel::get_degree(sim2) > 0))
...
vsupp2.edges <- do.call("c", sapply(ids.vsupp2,
                                    function(x) get.edgeIDs(sim2, x)))
