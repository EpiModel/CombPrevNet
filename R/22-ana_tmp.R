...
df %>%
  ...
  group_by(scenario, sim) %>%
  summarize(
    prep = mean(s_prep___ALL, na.rm = TRUE),
    elig = mean(s_prep_elig___ALL, na.rm = TRUE),
    prep_start    = mean(prep_start___ALL, na.rm = TRUE),
    part_prep    = mean(part_prep___ALL, na.rm = TRUE)
  ) %>%
  summarise(across(everything(), mean))

#look into part_prep & prep_start
